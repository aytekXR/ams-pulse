...
export function LoadingSpinner({ size = "md", label = "Loading…" }: Props) {
  ...
  return (
    ...
    >
      ...
      {/* s111 D3: secondary, not muted — 12px label is normal text and muted
          fails AA under the binding WCAG table. */}
      <span style={{ fontSize: 12, color: "var(--color-secondary)" }}>{label}</span>
    </div>
  );
}
