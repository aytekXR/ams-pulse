...
func New(ctx context.Context, backend, dsn, secretKey string) (*Store, error) {
	...
	s := &Store{db: db, backend: backend, cipherKey: cipherKey}
	return s, nil
}
...
func (s *Store) CreateToken(ctx context.Context, t APIToken) error {
	...
	_, err := s.db.ExecContext(ctx,
		`INSERT INTO api_tokens (id, user_id, kind, name, token_hash, scopes, expires_at, created_at)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
		t.ID, userID, t.Kind, t.Name, t.TokenHash, string(scopesJSON), t.ExpiresAt, t.CreatedAt)
	return err
}

// GetTokenByHash looks up a token by its SHA-256 hash (hot path for auth).
func (s *Store) GetTokenByHash(ctx context.Context, hash string) (*APIToken, error) {
	row := s.db.QueryRowContext(ctx,
		`SELECT id, user_id, kind, name, token_hash, scopes, expires_at, last_used_at, created_at
...
func (s *Store) ListTokens(ctx context.Context, kind string) ([]APIToken, error) {
	q := `SELECT id, user_id, kind, name, token_hash, scopes, expires_at, last_used_at, created_at
	      FROM api_tokens`
	var args []any
	if kind != "" {
		q += " WHERE kind = ?"
...
func scanAPIToken(row scanner) (*APIToken, error) {
	...
	if err := row.Scan(&t.ID, &userID, &t.Kind, &t.Name, &t.TokenHash, &scopesJSON,
...
// CreateAlertHistory inserts a history row.
