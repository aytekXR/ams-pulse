-- Pulse metadata store schema — migration 0001 (PostgreSQL)
-- Embedded copy of contracts/db/meta/postgres/0001_init.sql.
-- Sync command: cp contracts/db/meta/postgres/0001_init.sql server/internal/store/meta/sql/postgres_0001_init.sql
-- Source of truth: contracts/db/meta/0001_init.sql (SQLite)
...
--   PRAGMA lines             → dropped (no PG equivalent)
--   INTEGER (epoch-ms)       → BIGINT   (64-bit Unix epoch milliseconds)
--   INTEGER (bool 0/1 flag)  → INTEGER  (stays 0/1; BOOLEAN avoided for compat)
--   INTEGER (counter)        → BIGINT
--   INTEGER (window_s, rate_limit, interval seconds) → INTEGER (fits 32-bit)
--   REAL                     → DOUBLE PRECISION
--   TEXT                     → TEXT (unchanged)
--   AUTOINCREMENT            → not present in source (verified)
--   INSERT OR IGNORE         → INSERT ... ON CONFLICT (<pk_column>) DO NOTHING
--   strftime('%s','now')*1000 → (EXTRACT(EPOCH FROM NOW())::bigint * 1000)
--   CREATE UNIQUE INDEX IF NOT EXISTS → valid in PostgreSQL (unchanged)
--
-- Timestamps: all epoch-ms columns use BIGINT to hold 64-bit Unix ms values.
-- Booleans:   0/1 flag columns stay INTEGER so the application layer mapping
--             from the SQLite schema works without changes.

-- ── schema_migrations ─────────────────────────────────────────
CREATE TABLE IF NOT EXISTS schema_migrations (
    version     TEXT      NOT NULL PRIMARY KEY,
    applied_at  BIGINT    NOT NULL  -- Unix epoch ms
);
...
CREATE TABLE IF NOT EXISTS api_tokens (
    ...
    expires_at  BIGINT,                           -- Unix epoch ms; NULL = non-expiring
    last_used_at BIGINT,
    created_at  BIGINT    NOT NULL
);
...
CREATE TABLE IF NOT EXISTS ingest_tokens (
    ...
    rate_limit  INTEGER   NOT NULL DEFAULT 1000,  -- max events/min (fits 32-bit)
    expires_at  BIGINT,
    created_at  BIGINT    NOT NULL,
    revoked     INTEGER   NOT NULL DEFAULT 0      -- 0=active, 1=revoked (bool flag)
);
...
CREATE TABLE IF NOT EXISTS alert_rules (
    ...
    enabled             INTEGER NOT NULL DEFAULT 1,     -- 0=not evaluated, 1=active (bool flag)
    muted               INTEGER NOT NULL DEFAULT 0,     -- 0=notifications sent, 1=suppressed (bool flag)
...
CREATE TABLE IF NOT EXISTS alert_history (
    ...
    ts              BIGINT  NOT NULL,           -- Unix epoch ms
    metric          TEXT    NOT NULL,
    value           DOUBLE PRECISION,
    threshold       DOUBLE PRECISION,
    scope           TEXT    NOT NULL DEFAULT '{}',  -- JSON
    cooldown_until  BIGINT,                     -- Unix epoch ms
    group_key       TEXT,
    test            INTEGER NOT NULL DEFAULT 0  -- bool flag
);
...
CREATE TABLE IF NOT EXISTS license (
    ...
    valid           INTEGER NOT NULL DEFAULT 1,     -- bool flag
    expires_at      BIGINT,
    activated_at    BIGINT,
    updated_at      BIGINT  NOT NULL
);
...
CREATE TABLE IF NOT EXISTS probes (
    ...
    enabled         INTEGER NOT NULL DEFAULT 1,     -- bool flag
    last_result_id  TEXT,                           -- most recent ClickHouse result ID
    last_success    INTEGER,                        -- 0/1 from last run (bool flag)
    last_run_at     BIGINT,
    created_at      BIGINT  NOT NULL,
    updated_at      BIGINT  NOT NULL
);
...
INSERT INTO schema_migrations(version, applied_at)
    VALUES ('0001', (EXTRACT(EPOCH FROM NOW())::bigint * 1000))
