...
>
> **⚠ One honest consequence to know:** the S105 CODE fixes (Kafka topics/shapes, webhook official-payload, ProxyAuthorization) landed AFTER the v0.4.1 tag — the published 0.4.1 image does not contain them, while the docs on main now describe the fixed behavior. The clean resolution is **cutting v0.4.2** (next session's goal; mechanical — the new release-time version guard enforces the sweep). If you add the NPM_TOKEN secret BEFORE that tag, the same release automatically publishes `ams-pulse-beacon` to npm; without it, the tarball still attaches to the release and nothing fails.
>
...
>
> ### §1 — ★ THE ONE BLOCKING ACTION: flip the GHCR package to public (the image now EXISTS — you are unblocked)
> **Update (later same day):** PR #204 is merged, the `v0.4.1` tag is pushed, and `release.yml` built and pushed **`ghcr.io/aytekxr/ams-pulse:0.4.1`** (multi-arch, cosign-signed). The "wait for the image" precondition is met — the only step left is the visibility flip, which has no API and only you can do:
> GitHub → your profile → **Packages** → `ams-pulse` → **Package settings** → **Change visibility** → **Public**.
> The moment it is public, tell the loop (or it will pick it up on the next gate): it will run the anonymous clean-room install (`docker pull …:0.4.1` with no auth → quickstart → live dashboard) itself — that is the last mile of the review's blocker 1, and it does not need you.
> *(Note: the released `0.4.1` image is a marketplace/distribution artifact. Prod continues to run its own stamped source build and is unaffected by the tag; rolling prod to 0.4.1 is a separate, deliberate `deployment.sh` deploy, not something the tag does.)*
>
