...
| VD-23 | `api.IngestTracker` interface has wrong `Snapshot()` return type; `SetIngestTracker` never called | Major | OPEN — `handleIngestHealth` reads from a live snapshot path; `IngestTracker` interface is dead code |
| VD-X3-A | POST /admin/sources/{id}/test missing `reachable` field in response | Major | OPEN — spec is correct; handler fix (BE-02) deferred |
