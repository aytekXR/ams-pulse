...
| Anomaly detection | Mock-compatible | Mock-compatible | Mock-compatible | **Pipeline LIVE-VALIDATED; false-alarm rate is CI-measured** (modeled 0.43 false alarms/node-week from `TestAnomaly_FalseAlarmRate_ModeledTarget` — a modeled-workload test, not a live long-run) |
