...
| `node_degraded` | Fleet discovery | Fires while a node meets the degraded condition: CPU > 90%, memory > 90%, **or** 3 consecutive AMS API errors (`LiveNodeStats.Degraded()`) |
...
| ~~`node_degraded` alert metric is a placeholder~~ — **implemented** (D-087): the condition is `LiveNodeStats.Degraded()` (CPU/mem > 90% or ≥ 3 consecutive API errors). On **clusters** its reliability during an AMS API outage is still limited — see LIM-10 | Minor | Cluster hardening tracked post-0.4.3 |
