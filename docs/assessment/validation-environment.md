...
| AMS trial license lapsed 2026-07-12T12:09Z | `local_env.ams_trial_license_expiry` | Operator-waived; observe and report only. **S29/D-091 update:** blocked-scenario-list is no longer EMPTY. SRT ingest (TC-I-05-SRT) is license-blocked: SRTAdaptor "License is suspended" observed live 2026-07-13 (evidence: `qa/realams/evidence/S29-TC-I-05-SRT-20260713T215932Z/`). **S30/D-092 update:** the first post-lapse AMS restart (2026-07-13 22:21Z, crash + auto-restart) extended enforcement to RTMP: all new publishes refused ("License is suspended and not accepting connection"; evidence `qa/realams/evidence/S30-rtmp-license-block-20260713T2353Z/`). **S30 late-session update: the operator's new license (expires 2026-07-27T13:45Z) was applied via REST + AMS restart — RTMP ingest RESTORED and live-validated (teststream broadcasting, sweep byte-identical to the pre-expiry baseline). Enforcement model settled: lapse alone spares running streams and RTMP; a process restart under a lapsed license kills ALL new ingest; recovery = apply key + restart.** Blocked=[SRT ingest — pending a low-load window only: the first post-license TC-I-05 run cleared the license gate (handshake reached the ACF callback) but hit AMS's high-resource-usage guard at VPS load 14]. |
