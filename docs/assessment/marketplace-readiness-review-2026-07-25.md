...
4. **Infrastructure leak in a linkable doc:** `deploy/runbooks/upgrade-rollback.md:3` contains your VPS IP `<VPS_IP>`; nginx confs hardcode `beyondkaira.com` internals. Fine in-repo as reference, but the runbook is marked "READY / externally linkable" in the submission package — scrub the IP or de-list the doc.
