...
|---------|-----------------------------------|---------|----------|-------|
| **F1 — Real-Time Operations Dashboard** | **Overall** | **PARTIALLY** | — | Core ops dashboard validated against real AMS; node CPU/RAM **present since D-179** via `/rest/v2/system-resources` (only network throughput remains absent for standalone AMS via REST); webhook-based instant detection non-functional (REST poll covers the latency budget) |
...
| | Concurrent viewer counts within ±2% of AMS REST `broadcast-statistics` (standalone) | FULLY | TC-V-03 (S17-TC-V-03-20260711T120829Z; viewer_count within ±2); TC-V-04 (S17-TC-V-04-20260711T120522Z; rtmpViewerCount=0 across 20 samples at 5 s intervals; AV-16 CONFIRMED SAFE) | Viewer counts use inline BroadcastDTO fields (hlsViewerCount + webRTCViewerCount + rtmpViewerCount + dashViewerCount, normalize.go). The `amsclient.BroadcastStatistics()` method was dead code at runtime and was deleted in S26/D-088 (BUG-001 FIXED); inline counts are unchanged and accurate. |
| | Node health (CPU, RAM, network) visible | **DIFFERENTLY** *(CPU/RAM half superseded D-179 — now FULLY; network still absent)* | AV-06 (CONFIRMED ABSENT **from `/system-status`** — the endpoint AV-06 actually probed); AV-05 (CONFIRMED: os\_name, java\_version, processor\_count, version, role, status present); AV-15 (BLOCKED: Kafka path not deployed); **D-179 (2026-07-27): cpu\_pct, mem\_pct, disk\_pct LIVE-VERIFIED present on standalone AMS 3.0.3 via `/rest/v2/system-resources`** | **Superseded in part (D-179).** AV-06's finding was scoped to `/system-status`, and the conclusion drawn from it — that standalone AMS cannot report these at all — was never probed and turned out to be false. `/rest/v2/system-resources` returns CPU, memory and disk on standalone; Kafka and cluster-mode stats are alternatives, not requirements. **Still absent at every version:** `net_in_mbps` / `net_out_mbps` (no AMS REST route exposes them). Pulse still emits null for absent fields rather than false zeros (honest-absent design). See LIM-01 and DG-05 in `docs/assessment/documentation-gaps.md`. |
...
| Feature | Requirement / Acceptance Criterion | Verdict | Evidence | Notes |
|---------|-----------------------------------|---------|----------|-------|
| **F4 — Publisher and Ingest Health** | **Overall** | **PARTIALLY** | — | Bitrate, health score, and source-drop detection confirmed live; FPS always 0 on AMS 3.x REST; BUG-004 FIXED S20/D-082 (from/to/app/stream/node now honored; interval BUG-005 FIXED S21/D-083) |
...
| | Ingest degradation visible within 15 s | FULLY | CI: 250.8 µs in-process detection (C-W2-06); TC-I-04 (S18-TC-I-04-20260711T145154Z; 4/4 PASS — BUG-004 now FIXED S20/D-082; workaround no longer required) | Detection is in-process (essentially instantaneous). The REST poll propagation delay adds up to 5 s in the worst case, well within the 15 s budget. |
| | Per-source historical charts (windowed time range) | FULLY | TC-I-04 (S18; 4/4 PASS; BUG-004 FIXED S20/D-082 — handler now reads from/to/app/stream/node; BUG-005 FIXED S21/D-083 — interval param now honored; new tests: TestBUG004_IngestHealth_HonorsTimeRange (13 subtests), TestBUG005_IngestHealth_HonorsBucketInterval (5 subtests)) | BUG-004 root cause: handleIngestHealth never parsed from/to, IngestTimeseries received no time window. Fix: parseTimeParam helper (zero time.Time on absent input, not the 7-day default of parseTimeRange); From/To plumbed into IngestTimeseriesParams; app/stream/node filter stream set. BUG-005 fix: parseBucketInterval maps hour→3600, day→86400, absent/invalid→0 preserving 60 s default required by F4 "15 s degradation visibility" criterion. Contract UNCHANGED (gen:api + git diff --exit-code clean). Production impact: web client getIngestHealth was sending from=now-15min&to=now on every page load — all-time era-mixed buckets were the live dashboard view. |
...
| Feature | Requirement / Acceptance Criterion | Verdict | Evidence | Notes |
|---------|-----------------------------------|---------|----------|-------|
| **F5 — Alerting and Incident Automation** | **Overall** | **PARTIALLY** | — | Core alert pipeline validated live (stream offline, bitrate floor, viewer count); CPU/mem/disk alerts **work on standalone since D-179** (`/rest/v2/system-resources`); error rate and rebuffer ratio alerting not live-validated |
...
| | Node CPU/disk alerts | **DIFFERENTLY** *(superseded D-179 — these alerts now fire on standalone)* | AV-06 (CONFIRMED ABSENT **from `/system-status`**); TC-H-06 (S18-TC-H-06-20260711T141402Z; 3/3 PASS — standalone CPU anomaly correctly returns empty, not an error, **when the metric is genuinely absent**); **D-179: cpu\_pct / mem\_pct / disk\_pct live-verified present via `/rest/v2/system-resources`** | **Superseded (D-179).** The old note — "alert rules on these metrics cannot fire without data" — was true only of the `/system-status` path AV-06 probed. Pulse now polls `/rest/v2/system-resources`, so CPU, memory and disk alert rules **do** fire on standalone AMS with no Kafka (LIM-01, DG-05 both updated). The honest-absent behaviour TC-H-06 pinned still applies on an AMS that does not serve the route, where Pulse falls back and the evaluator raises no false alarms. Kafka (AV-15 BLOCKED) is now an alternative path, not the only one. |
...
| Feature | Requirement / Acceptance Criterion | Verdict | Evidence | Notes |
|---------|-----------------------------------|---------|----------|-------|
| **F6 — Usage and Billing Reports** | **Overall** | **PARTIALLY** | — | Viewer-minutes and peak concurrency confirmed; egress is a model-based estimate (method disclosed); recording_gb FIXED S23/D-085 (BUG-002 — VoD REST poll + `mv_recording_1d`, live-validated TC-REC-01); tenant mapping still not live-validated |
...
| | Recording storage (recording\_gb) | FULLY | BUG-002 FIXED S23/D-085: TC-REC-01 (S23-TC-REC-01-20260712T151721Z; 3/3 PASS live vs real AMS — recording\_gb=0.003126 vs AMS ground truth 3,125,555 bytes = 0.0031256 GB, reconciliation within 0.02%); TestIntegration\_MvRecording (CI: sum(recording\_bytes)=2×size, bucket=VoD creation day); 8 poller unit tests incl. restart-resume + dedup-bypass pins | Fixed by the VoD REST poll fallback (design note `bugs/BUG-002-design-note-vod-rest-poll.md`): the restpoller polls `/{app}/rest/v2/vods/list` every 12th tick (60 s at default), dedupes via a persistent seen-set keyed on the stable AMS `vodId` (`vod_poll_state` meta table, migration 0003) and emits `recording_ready` events; new ClickHouse MV `mv_recording_1d` (migration 0009) rolls `server_events.recording_size` into `rollup_usage_1d.recording_bytes`. Event TS = VoD creationDate, so billing buckets attribute to the creation day. The webhook path stays fail-closed (unchanged; D-066/O3). Ground truth: the S17 pulse-test fixture VoD (the pre-S17 app reset wiped older VoD inventories). |
...
| | Per-node and aggregate views | FULLY | TC-FL-01 (S17-TC-FL-01-20260711T115050Z; 7/7 PASS — standalone fleet card); TC-FL-02 (S17-TC-FL-02-20260711T120431Z; 4/4 PASS — version=3.0.3 Enterprise Edition confirmed; AV-05 node card fields confirmed) | Fleet node card fields confirmed: os\_name=Linux, os\_arch=amd64, java\_version=17, processor\_count=6, version=3.0.3, role=standalone, status=up (AV-05). Note: cpu\_pct, mem\_pct were absent from `/system-status` (AV-06); **present since D-179** via `/rest/v2/system-resources`. |
...
| | Node up/down alerts | PARTIALLY | BUG-011 FIXED S25/D-087 (EvictStaleNodes wired in serve.go via wireNodeEviction goroutine; failure path no longer refreshes LastSeenAt — see `docs/assessment/bugs/BUG-011-evictstalenodes-never-wired.md`); RED→GREEN unit tests: TestBUG011\_NodeEviction\_Wired (serve\_wiring\_test.go), TestAggregator\_FailureStreak\_LastSeenAtFrozen, TestAggregator\_FailureStreak\_EvictStillWorks (aggregator\_node\_streak\_test.go); TC-H-04/05 PASS (alert evaluation pipeline functional); TC-F-01/02 PASS (stream lifecycle detection via REST poll) | BUG-011 FIXED S25/D-087: EvictStaleNodes was implemented (aggregator.go:202–224) but never called in production before D-087. The wireNodeEviction goroutine (serve.go) now calls it with threshold=3×PollInterval; the failure-streak path no longer refreshes LastSeenAt, so rung-3 eviction can fire. Unit and serve-wiring tests RED→GREEN (see BUG-011 bug doc for full red/green output). A live node-offline scenario against the real AMS has NOT been run (single-node VPS constraint); node\_down path is structurally correct and unit-proven but not live-validated against a frozen or restarted AMS node. |
...
| Feature | Requirement / Acceptance Criterion | Verdict | Evidence | Notes |
|---------|-----------------------------------|---------|----------|-------|
| **F8 — Data API, Prometheus Endpoint and Exports** | **Overall** | **PARTIALLY** | — | REST API and token authentication confirmed live; Prometheus /metrics endpoint not live-validated against real AMS; scheduled S3 export not verified; BUG-004/005 FIXED S20–S21; BUG-006/007/010 FIXED S22; BUG-008 from/to FIXED S24/D-086 (anomaly_flag_events + flagHistoryBridge); BUG-009 tenant on GET /live/overview + /live/streams remain known-violations (F6 backlog); conformance registry: 37 probes / 2 known-violations / 47 exempt |
...
| | Documented OpenAPI spec; API parity with dashboard data | PARTIALLY | N36 (CI: 51/52 operations response-body conformant; GET /live/ws waived; D-060); BUG-004/005 FIXED S20–S21; BUG-006 FIXED S22 (limit+cursor on 8 list endpoints); BUG-007 FIXED S22 (cursor on alerts/history + probes/results); BUG-010 FIXED S22 (format=csv declared in contract; minSpecParams 85→86); BUG-008 FIXED S22+S24 (app/stream/limit/cursor probed S22; from/to probed S24/D-086 via anomaly_flag_events + flagHistoryBridge); BUG-009 PARTIALLY FIXED S22 (LiveStreams cursor + stability sort fixed; tenant ×2 remain known-violation → F6 multi-tenancy) | The OpenAPI spec exists; response-body conformance is 51/52 (WebSocket upgrade waived). Parameter conformance gate added S21/D-083 (param_conformance_test.go: 85→86 declared params, all registered). Post-S24 census: 37 probes / 2 known-violations / 47 exempt. Remaining known-violations: GET /live/overview ?tenant and GET /live/streams ?tenant (BUG-009 — domain.LiveSnapshot has no tenant assignment; F6 multi-tenancy backlog). Both are pinned with bugRef in the conformance registry. |
...
| | Baseline-deviation flags on viewers and bitrate | FULLY | CI: modeled 0.43 false alarms/node-week at default sensitivity (σ=4.0, hysteresis=10; TestAnomaly\_FalseAlarmRate\_ModeledTarget); TC-AN-03 (S18-TC-AN-03-20260711T141259Z; 2/2 PASS — CPU/mem anomaly correctly returns empty for standalone); AV-12 (CONFIRMED: GET /api/v1/anomalies → empty list for standalone AMS) | False-alarm rate well below the 1/node-week threshold. The empty anomaly list for standalone is correct behavior (no CPU/mem data source). |
| | Baseline-deviation flags on errors and rebuffering | MISSING | TC-AN-05 (S18-TC-AN-05-20260711T141300Z; 3/3 PASS — confirms error\_rate not tracked) | error\_rate and rebuffer\_ratio are confirmed not implemented as anomaly signals in the current evaluator. The PRD specifies anomaly detection on "viewers, errors and rebuffering"; error and rebuffer anomaly signals are absent. S25/D-087 assessed: SPARSITY GATE — prod `beacon_events` = 2 rows / 1 stream (`u3-smoke`, 2026-07-10); `realams` = 0 rows; all-zero baselines ⇒ epsilon-floor makes the FIRST real rebuffer event an instant false alarm (violates PRD F9 '<1 false alarm/node-week'); `rollup_qoe_1h` buckets ACCUMULATE within the hour ⇒ 30 Welford ticks read non-independent samples (windowing redesign needed: minute-granularity or tick-deltas). Re-assess when a real beacon deployment shows sustained multi-viewer traffic AND a sub-hour windowing design exists. |
| | CPU/mem anomaly detection for standalone AMS | DIFFERENTLY | AV-06 (CONFIRMED ABSENT: cpu\_pct/mem\_pct not available via REST for standalone); TC-H-06 (S18; 3/3 PASS — correctly returns empty); AV-12 (CONFIRMED: empty anomaly list) | CPU/mem data is unavailable for standalone AMS via REST (DG-05). The anomaly evaluator correctly returns no findings rather than false alarms (honest-absent design — made structural in S26/D-088: per-metric presence flags now prevent unreported cpu/mem/disk observations from entering the Welford baselines at all, and pre-existing zero-mean baseline rows are swept at detector startup; before S26 the empty result was correct only because standalone values never varied from the poisoned zero-mean baseline). CPU/mem anomaly detection is available in cluster mode if node stats are exposed via the cluster API. |
| | False alarm rate < 1/node-week at default sensitivity | FULLY | CI: 0.43/node-week at σ=4.0, hysteresis=10 (TestAnomaly\_FalseAlarmRate\_ModeledTarget); N19 criterion met | Target met with a comfortable margin (57% below threshold). |
...
| Feature | Requirement / Acceptance Criterion | Verdict | Evidence | Notes |
|---------|-----------------------------------|---------|----------|-------|
| **F10 — Synthetic Viewer Probes (Phase 3)** | **Overall** | **FULLY** | — | All implemented probe types validated live against AMS 3.0.3; rtt/jitter/loss live-proven for WebRTC (D-075); BUG-003 FIXED S20/D-082 (respawn-on-unchanged-config eliminated; probe timing now truly periodic; filed hypothesis wrong — actual root cause was 60 s refresh loop unconditionally respawning all probes); Phase 3 item delivered ahead of Phase 3 |
...
| | Probe result interval accuracy | FULLY | TC-P-07 (S18; 4/4 PASS; BUG-003 FIXED S20/D-082 — workaround no longer required; regression pin: TestBUG003_NoRespawnOnUnchangedConfig, confirmed RED on pre-fix code with exact evidence signature "expected 4 fires got 5", GREEN after fix; TestBUG003_N24_FirstFireImmediate guards <100 ms first-result budget) | BUG-003 actual root cause (filed hypothesis of "immediate-on-create goroutine + periodic ticker" was WRONG): Runner.Run reloaded probe list on a 60 s refresh ticker and called spawnProbe for every probe on every tick; spawnProbe unconditionally cancelled and respawned even unchanged probes; respawned goroutine fires after jitter(interval) and prod leaves MaxJitterFraction=0 → fires immediately, 0–1 ms on top of the original goroutine's phase-aligned fire. Fix: probeEntry stores domain.ProbeConfig; spawnProbe returns early on whole-struct equality (changed config still respawns; removed probe still cancels). The three filed fix suggestions (insert-time dedup, results-API dedup, per-probe mutex) were rejected — all hide the duplicate without fixing the phase reset. |
...
| N19 | F9 anomaly detection false-alarm rate < 1/node-week at default sensitivity | < 1 false alarm / node-week | **0.43/node-week** (σ=4.0, hysteresis=10) | FULLY | CI: TestAnomaly\_FalseAlarmRate\_ModeledTarget; ARCHITECTURE.md §4 |
...
| N36 | OpenAPI response-body conformance: all operations except GET /live/ws (WebSocket upgrade, 101 response, waived) | 51 of 52 operations conformant | **51/52** response-body conformant; BUG-004 FIXED S20 (parameter violation, not response-body); parameter conformance gate added S21/D-083 (37 probes / 2 known-violations / 47 exempt at S24 close); remaining known-violations: BUG-009 ?tenant ×2 on /live (F6 backlog) | FULLY | CI: OpenAPI conformance test suite (D-060 formalized; VD waived GET /live/ws); param_conformance_test.go (S21/D-083 — enumerates all 86 declared query params, fails on unregistered; BUG-004/005/006/007/010 fixed and promoted to probe; BUG-008 from/to promoted to probe S24/D-086; BUG-009 tenant ×2 remain known-violation with bugRef) |
...
|-----|----------|-------------------|--------|
| BUG-001 | Low | F1 (viewer count detail path — no user impact, counts work via inline poll) | **FIXED S26/D-088** (dead code deleted; live pipeline never affected) |
| BUG-002 | High | F6 (recording\_gb always 0 — VoD REST never polled; vodReady webhook blocked on AMS 3.0.3 (cannot HMAC-sign hooks)) | Confirmed; P0 roadmap fix: VoD REST poll fallback (design note: `bugs/BUG-002-design-note-vod-rest-poll.md`); S23 primary |
| BUG-003 | Medium | F10 (probe scheduler near-duplicate rows every 60 s — unconditional respawn on refresh tick) | **FIXED S20/D-082** (PR #32): probeEntry stores config; spawnProbe returns early on unchanged config; 4 regression tests |
| BUG-004 | Medium | F4 (windowed ingest health queries return era-mixed data); F8 (OpenAPI contract violation) | **FIXED S20/D-082** (PR #32): handleIngestHealth reads from/to/app/stream/node; 13 subtests TDD red→green |
| BUG-005 | Medium | F4/F8 (GET /qoe/ingest `interval` param declared but ignored — 60 s buckets always returned) | **FIXED S21/D-083** (PR #33): parseBucketInterval maps hour→3600, day→86400, absent→0; 5 subtests |
| BUG-006 | Medium | All 8 list endpoints (limit+cursor declared but unimplemented; store layer unbounded) | **FIXED S22/D-084** (PR #34): keyset cursors on all 8 store methods; limit+1 sentinel; 35 conformance probes |
| BUG-007 | Low–Medium | F5/F10 (cursor dropped in GET /alerts/history + GET /probes/{id}/results) | **FIXED S22/D-084** (PR #34): cursor added to store.ListAlertHistory + qsvc.QueryProbeResults; promoted from exempt to real probes |
| BUG-008 | High | F9 (GET /anomalies drops from/to/app/stream/limit/cursor — all six declared filters) | **FIXED S22+S24 (D-084 Group A, D-086 Group B)**: app/stream/limit/cursor (Group A) fixed handler-side S22; from/to (Group B) fixed S24 via `anomaly_flag_events` ClickHouse table + ADR-0009 + `flagHistoryBridge` wired in serve.go |
| BUG-009 | Medium | F6/F1 (GET /live/overview + /live/streams: tenant accepted by handler but dropped in query layer; LiveStreams cursor stubbed) | **PARTIALLY FIXED S22/D-084** (PR #34): LiveStreams cursor decoded + stability sort added; tenant ×2 remain known-violation → domain.LiveSnapshot has no tenant assignment (F6 multi-tenancy backlog) |
| BUG-010 | Low | F2/F8 (GET /analytics/audience reads ?format=csv but param undeclared in OpenAPI spec) | **FIXED S22/D-084** (PR #34): format enum [json,csv] + text/csv 200 declared; gen:api regenerated; minSpecParams 85→86 |
...
Note: There is no separate BUG-005 file; BUG-005 is documented as a residual section at the bottom of `BUG-004-qoe-ingest-ignores-from-to.md`.
