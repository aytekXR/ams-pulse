...
| Feature | Requirement / Acceptance Criterion | Verdict | Evidence | Notes |
|---------|-----------------------------------|---------|----------|-------|
| **F8 — Data API, Prometheus Endpoint and Exports** | **Overall** | **PARTIALLY** | — | REST API and token authentication confirmed live; Prometheus /metrics endpoint not live-validated against real AMS; scheduled S3 export not verified; BUG-004/005 FIXED S20–S21; BUG-006/007/010 FIXED S22; BUG-008 from/to on GET /anomalies remain known-violations (S23 ADR); BUG-009 tenant on GET /live/overview + /live/streams remain known-violations (F6 backlog); conformance registry: 35 probes / 4 known-violations / 47 exempt |
...
| | Documented OpenAPI spec; API parity with dashboard data | PARTIALLY | N36 (CI: 51/52 operations response-body conformant; GET /live/ws waived; D-060); BUG-004/005 FIXED S20–S21; BUG-006 FIXED S22 (limit+cursor on 8 list endpoints); BUG-007 FIXED S22 (cursor on alerts/history + probes/results); BUG-010 FIXED S22 (format=csv declared in contract; minSpecParams 85→86); BUG-008 PARTIALLY FIXED S22 (app/stream/limit/cursor probed; from/to remain known-violation — no persistent flag-event store; S23 ADR); BUG-009 PARTIALLY FIXED S22 (LiveStreams cursor + stability sort fixed; tenant ×2 remain known-violation → F6 multi-tenancy) | The OpenAPI spec exists; response-body conformance is 51/52 (WebSocket upgrade waived). Parameter conformance gate added S21/D-083 (param_conformance_test.go: 85→86 declared params, all registered). Post-S22 census: 35 probes / 4 known-violations / 47 exempt. Remaining known-violations: GET /anomalies ?from and ?to (BUG-008 architectural gap — in-memory detector has no flag-event history; S23 designs ClickHouse anomaly_flag_events table); GET /live/overview ?tenant and GET /live/streams ?tenant (BUG-009 — domain.LiveSnapshot has no tenant assignment; F6 multi-tenancy backlog). All four are pinned with bugRef in the conformance registry. |
...
| N36 | OpenAPI response-body conformance: all operations except GET /live/ws (WebSocket upgrade, 101 response, waived) | 51 of 52 operations conformant | **51/52** response-body conformant; BUG-004 FIXED S20 (parameter violation, not response-body); parameter conformance gate added S21/D-083 (35 probes / 4 known-violations / 47 exempt at S22 close); remaining known-violations: BUG-008 ?from/?to on /anomalies + BUG-009 ?tenant ×2 on /live | FULLY | CI: OpenAPI conformance test suite (D-060 formalized; VD waived GET /live/ws); param_conformance_test.go (S21/D-083 — enumerates all 86 declared query params, fails on unregistered; BUG-004/005/006/007/010 fixed and promoted to probe; BUG-008 from/to + BUG-009 tenant ×2 retained as known-violation with bugRef) |
...
| BUG-008 | High | F9 (GET /anomalies drops from/to/app/stream/limit/cursor — all six declared filters) | **PARTIALLY FIXED S22/D-084** (PR #34): app/stream/limit/cursor (Group A) fixed handler-side; from/to (Group B) remain known-violation — architectural gap: in-memory detector has no flag-event history; S23 designs anomaly\_flag\_events table (ADR required) |
