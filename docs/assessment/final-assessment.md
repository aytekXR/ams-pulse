...
- The **AMS trial license** lapsed 2026-07-12T12:09Z. All core sessions (S17–S28)
  were completed before expiry (operator-waived). License renewed S30; new key
  (expires 2026-07-27T13:45Z) applied + AMS restarted; ingest RESTORED. **S31/D-093
  SRT validation PASS (2026-07-14T02:29:45Z):** TC-I-05-SRT passed 2/2 assertions
  (evidence: `qa/realams/evidence/TC-I-05-SRT-20260714T022945Z/`); blocked-scenario
  list is now EMPTY. SRT publishType live-observed: AMS reports `publishType="RTMP"`
  for SRT ingest (Pulse mirrors AMS verbatim — not a Pulse bug; see F5/D-093).
...
| 9 | Marketplace revenue-share terms agreed | NEEDS-OPERATOR-CONTACT | *(v0.3.0-era row — superseded)* The PRD's 20–30% target is obsolete: Ant Media publicly states first-year marketplace revenue is 100% the vendor's; post-year-1 terms are to be confirmed in writing at the developer meeting (see `submission-process.md` A4). |
| 10 | Listing category, screenshots, and description copy | NEEDS-OPERATOR-CONTACT | *(v0.3.0-era row — superseded)* The submission copy is now `docs/marketplace/listing.md` (D-173: category proposed, long description, tier table, noncommercial disclosure); all 6 listing screenshots + 9 user-guide shots are captured at 1920×1080. Only the operator submission step remains. |
...
| 12 | Semantic versioning and release artifacts | PASS | v0.4.1 released; GHCR package made public (D-172, S105); cosign-signed multi-arch images, SBOM + provenance, CI-gated tag pipeline. *(Updated D-173/D-174: releases now also attach `pulse-linux-{amd64,arm64}` binaries + SHA256SUMS, the SDK tarball, and the Helm chart, which is additionally pushed to GHCR OCI.)* |
...
> **PREMISE SUPERSEDED (D-179, 2026-07-27) — the section below is kept as the
> historical record.** Its verified fact still holds: `/rest/v2/system-status`
> omits CPU/memory/disk. Its *conclusion* — that Kafka (or cluster mode) is
> therefore the only route — was wrong. `GET /rest/v2/system-resources` returns
> all three on a standalone node, exists at `ams-v2.10.0` and every version
> since, and was live-verified against standalone AMS 3.0.3 on 2026-07-27. The
> Fleet gauges, the CPU/mem/disk alert rules and the anomaly baselines all work
> with no broker. See `docs/known-limitations.md` LIM-01. This is the canonical
...
| ~~**P1**~~ **✅ DONE — D-179 (2026-07-27), and no Kafka was needed** | ~~**Standalone CPU / mem / disk via Kafka**~~ — **premise superseded.** `GET /rest/v2/system-resources` returns `cpuUsage`, `systemMemoryInfo` and `fileSystemInfo` on a standalone node; live-verified against AMS 3.0.3 Enterprise, capture committed at `server/pkg/amsclient/testdata/system_resources_real_v303.json`. LIM-01 closed. Kafka (AV-15, still pending live validation) now buys only per-stream ingest metrics — FPS, keyframe interval, jitter, packet loss | ~~High~~ **Resolved** — Fleet gauges, CPU/mem/disk alert rules and anomaly baselines all work on the most common deployment profile, with no broker | ~~High~~ **Low as shipped** — one preferred endpoint ahead of the old one, with an honest 404/405 fallback to `/system-status` | High — the "fleet health" positioning claim now holds on standalone |
...
| ~~**P1 — BLOCKED**~~ **✅ DONE — S31 (D-093, 2026-07-14T02:29:45Z)** | **SRT loss validation against live SRT ingest** — TC-I-05-SRT PASS (2/2 assertions): `status=broadcasting`, `bitrate=1148432 bps`, `packetLostRatio=0.0`, `packetsLost=0`, Pulse `packet_loss_pct=0`. Evidence: `qa/realams/evidence/TC-I-05-SRT-20260714T022945Z/`. Post-ARQ observation confirmed: `packetLostRatio=0` on a clean 0% loss run (expected). SRT publishType observed: AMS reports `publishType="RTMP"` for SRT ingest (F5 finding, D-093). **Additional finding:** AMS SRTAdaptor requires plain streamId form `LiveApp/<id>`; ACF prefix forms (`#!::h=...`, `#!::r=...`) are rejected — scenario corrected. Blocked-scenario list: **EMPTY**. | Medium — documented (DG-18 + AMS-INTEGRATION.md note); Pulse reports what AMS reports | None — test-only; doc clarification complete | Low — correctness documentation complete |
