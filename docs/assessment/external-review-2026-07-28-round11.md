...
| M-03 "the report scheduler already had this gate… **no report-side residual**… prober / reports / alerts are now uniform" | **UPHELD — and it surfaced a product inconsistency to report** | The claim stands as filed. The anomaly detector (`serve.go:614`, `anomaly.go:310 → Run`) *is* a fourth tier-gated background loop with no runtime gate, and this session first treated that as the same gap and closed it at all three points. **CI refuted that:** e2e scenario A5 mints an ephemeral **Business** licence, POSTs an anomaly rule expecting **201**, and asserts the anomaly alert fires — green for many sessions. So anomaly *alerting* at Business is the product's live-validated behaviour, the detector being ungated is **correct**, and what actually disagrees is the pricing prose (`overview.md:220`, `listing.md:138`, PRD §7.11) which calls anomaly detection Enterprise-only while only `GET /anomalies` enforces it. All three gates reverted; the inconsistency is filed for the operator, since one answer changes what is advertised and the other stops alerts that below-Enterprise tenants receive today |
...
- **The anomaly tier inconsistency** — reported, not resolved. Enforcing the advertised
  Enterprise-only boundary was implemented and reverted when e2e A5 proved anomaly alerting at
  Business is live-validated behaviour. Either the prose or the code is wrong and both fixes are
  one line; the choice changes either what is advertised or what existing tenants receive, so it
  is the operator's.
