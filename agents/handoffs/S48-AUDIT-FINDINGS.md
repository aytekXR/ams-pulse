...
- **loc:** `server/internal/collector/dedup.go:18`  ·  **lens:** poller-sessions-agg  ·  status: ✅ DONE (D-111, PR #95, prod v0.4.0-39-gc08ad6a)
...
- **loc:** `server/internal/collector/aggregator/aggregator.go:562`  ·  **lens:** poller-sessions-agg  ·  status: ✅ DONE (D-111, PR #95, prod v0.4.0-39-gc08ad6a)  ·  **NOTE:** shipped the pointer-equality guard (fixes eviction-of-visible / Case A). Residual last-write shadowing when the *visible* stream ends (Case B) is the documented last-write-wins behavior, self-heals on the next stats event; a full compound-key rekey would break the bare-stream_id groupKey lookup in alert/evaluator.go — deliberately out of scope. See D-111.
...
- **loc:** `server/pkg/amsclient/client.go:475`  ·  **lens:** amsclient  ·  status: ✅ DONE (D-112, PR #97, prod v0.4.0-41-g60f2a13)  ·  **NOTE:** escaped `streamID` ONLY (the sole publisher-controlled path segment); `app` left raw per the audit's own refute of app/nodeID escaping. The other 4 path-builders take only `app`+numeric args — no streamID — so this was a single fix point.
...
- **loc:** `server/internal/reports/scheduler.go:169`  ·  **lens:** reports  ·  status: ✅ DONE (D-113, PR #99, prod v0.4.0-43-g7c206a9)  ·  **NOTE:** shipped as a pure `previousCalendarMonthUTC(now)` helper (inclusive [first, last]-of-prev-month), bundled with [15]. Verified the inclusive `bucket <= ?` bound against accounting.go:180/204/371.
...
- **loc:** `server/internal/cluster/discovery.go:264`  ·  **lens:** cluster  ·  status: ✅ DONE (D-114, PR #101, prod v0.4.0-45-g0ab487f)  ·  **NOTE:** added `n.Status != "down"` (degraded still counts). Verified impact at aggregator.go:342-346. Review refuted a "split-brain double-count" concern (single origin-pointed restpoller → no second stream_stats series). **★ This was the last of the 6 HIGH findings.**
...
- **loc:** `server/internal/collector/ingest/health.go:172`  ·  **lens:** beacon-ingest  ·  status: ✅ DONE (D-115, PR #103, prod v0.4.0-47-gd32b165)  ·  **NOTE:** `if now.IsZero()` → `if ev.TS <= 0` (fixed a broken existing guard — time.UnixMilli(0) is 1970, not Go zero). Mutation-proven; self-reviewed (mechanical).
...
- **loc:** `server/internal/collector/webhook/webhook.go:160`  ·  **lens:** webhook-hmac  ·  status: ✅ DONE (D-123, PR #117, prod v0.4.0-61-g28812db) · **NOTE:** shipped a **backward-compatible, opt-in** replay check — `PULSE_WEBHOOK_REQUIRE_TIMESTAMP` (default **off**) + `PULSE_WEBHOOK_TIMESTAMP_SKEW` (default 5m). Product-viability verified FIRST: AMS lifecycle webhooks are UNSIGNED (`AMS-INTEGRATION.md §4.5`), so `X-Ams-Signature` is a Pulse-defined contract → Pulse can extend it. OFF path = bare-body HMAC, byte-for-byte the original (zero ingest risk). ON path = require fresh `X-Ams-Timestamp` (±skew window) + bind the **canonical** decimal ts into the HMAC (`sha256=hex(HMAC(ts+"."+body))`). Full suite 24/24; mutation-proven ×3 (window `||`→`&&`; binding revert; boundary `<`→`<=`); backward-compat test; **3-lens adversarial review** (10 agents, 7 confirmed/0 refuted/0 blockers) → addressed 5 (env-wire skew, canonical ts, clearer log, plural B7 comment, boundary+non-canonical tests, docs), **deferred 1** (per-source `SourceRequireTimestamp` override map — YAGNI for an opt-in guard on an unused path, needs meta-store plumbing). Docs: `AMS-INTEGRATION.md §4.7`. **★ This was the LAST open S48 finding — audit COMPLETE (14 shipped, 2 deferred).**
...
- **loc:** `server/internal/collector/restpoller/restpoller.go:455`  ·  **lens:** poller-sessions-agg  ·  status: ✅ DONE (D-116, PR #105, prod v0.4.0-49-g6d60f53)  ·  **NOTE:** decoupled eviction (`stale`, all disappeared app-scoped keys) from emission (`ended`, broadcasting-only). Mutation-proven; D-029 app-scoping invariant guarded by the existing multiapp test.
...
- **loc:** `server/internal/reports/accounting.go:350`  ·  **lens:** reports  ·  status: ✅ DONE (D-117, PR #107, prod v0.4.0-51-ge5577f7)  ·  **NOTE:** shipped a **3-way** report-level disclosure (`bitrate_x_watch_time` / `ams_rest_stats_byte_counter` / new **`mixed`**) tracked across the included rows — broader than the audit's literal "any→byte-counter", which would over-claim precision on a mixed daily report (`Totals.EgressGB` blends both methods → the mirror of the original bug). Free-text string (no enum); OpenAPI + `schema.d.ts` document `"mixed"`. Mutation-proven ×3 (incl. a tenant-exclusion regression guard); 3-lens review, 0 confirmed.
...
- **loc:** `server/internal/query/query.go:1084`  ·  **lens:** clickhouse  ·  status: ⏸️ DEFERRED (D-121, S59, 2026-07-16 — no fix shipped) · **NOTE:** re-verification CONFIRMED the columns are wrong (`viewer_count`/`ts`, not `viewers`/`event_time` per `0001_init.sql:48,58`) AND that `AnomalyBaselineForMetric` is **DEAD CODE** — `grep -r '\.AnomalyBaselineForMetric' server/` hits only `wave3_anomaly_query_test.go`; no endpoint or the live `anomaly.Detector` (meta-store Welford baselines) reaches it. This exact latent bug was already listed and **deliberately deferred by D-087** ("Latent bug (scout catch, A2 to assess): query.go:1081"; F9 ClickHouse-baseline path GATED on real traffic per D-087's sparsity ruling). **Ruling: DEFER, do not fix** — fixing dead code against an explicit deferral is churn with zero prod impact, and a piecemeal column fix would be incomplete (needs the default-branch metric-allowlist redesign D-087 describes, done TOGETHER when the function is wired). Shipped: an inline deferral pin at `query.go:1092` naming the wrong columns, the correct fix, and the wire-it-first gate. **No prod roll** (comment-only, byte-identical binary). Also dodges the VACUOUS-test trap the finding warned of (fake conn ignores SQL text — a real test needs a SQL-text seam or real CH, worth building only alongside the wiring).
...
- **loc:** `contracts/db/clickhouse/0001_init.sql:358`  ·  **lens:** clickhouse  ·  status: ⏸️ DEFERRED (D-122, S60, 2026-07-16 — impact refuted, no migration shipped) · **NOTE:** the mechanism is real (`peak_concurrency` is NOT in the SummingMergeTree sum-list, so it's kept-from-one-row after a merge) BUT the impact is REFUTED: a whole-repo grep confirms **nothing reads `rollup_usage_1d.peak_concurrency`** — every peak READ comes from an AggregatingMergeTree via `maxMerge` (billing: `accounting.go:389-412` → `rollup_concurrency_1d`; analytics: `query.go:285` → `rollup_audience_1h/1d`; web `ReportsPage.tsx:866` gets the API value fed from those). `accounting.go:209-210` literally documents the column as an unread "session-count proxy, not true concurrency." This is a human-approved, integration-tested design — **D-018 CR-VD38** created `0002_concurrency_rollup.sql` (`rollup_concurrency_1d`, `maxState(viewer_count)` from `server_events`) for exactly this, `TestAccountant_CHIntegration` proves "TRUE windowed max, drift 0.0000%" (D-019), and D-018 states "Do NOT edit `0001_init.sql`." **Ruling: DEFER** — the audit's fix (add to the sum-list) would be inert (no reader), semantically wrong if ever read (summing `toUInt32(1)`/session = session-count, not peak), and risky (live `ALTER … MODIFY ENGINE` on the billing table) for zero benefit. Also note the migration lineage is already at **0010**, not 0004 — "next=0005" in the plans was the meta-store audit_log lineage. No code/DDL change (the live read-path is already pinned at `accounting.go:209-211`); no prod roll.
...
- **loc:** `server/internal/store/clickhouse/clickhouse.go:550`  ·  **lens:** clickhouse  ·  status: ✅ DONE (D-118, PR #109, prod v0.4.0-53-g500aabb)  ·  **NOTE:** hoisted `PrepareBatch` above the double loop + one `Send` after all `Append`s (mirroring `insertServerEvents`/`insertViewerSessions`) → the flush is atomic, so a mid-batch failure commits nothing and the flusher's all-or-nothing `inserted`/`dropped` accounting matches reality. Mutation-proven by splicing the exact original per-item func back (2 distinguisher tests redden: `got 3 sends want 1`; `got 1 partial row want 0/3`); added a default-off send-failure hook to the test mock. Self-review (mechanical).
...
- **loc:** `server/internal/collector/beacon/beacon.go:352`  ·  **lens:** beacon-ingest  ·  status: ✅ DONE (D-120, PR #113, prod v0.4.0-57-g36c16ed)  ·  **NOTE:** replaced the `len(body) >= maxBodyBytes-1` heuristic with `errors.As(err, &*http.MaxBytesError)` so a mid-body read error (ECONNRESET) on a sub-limit body returns 400, not 413. **Did NOT** remove the post-read `len(body) >= maxBodyBytes` check — the audit wrongly called it unreachable, but `http.MaxBytesReader` does not error on a body of *exactly* `maxBodyBytes`, so that check catches the exact-boundary case (verified CORE narrower than the audit). Mutation-proven (revert to heuristic → new test reddens `got 413 want 400`, `OverSize_413` green). Self-review (mechanical, LOW).
...
- **loc:** `server/internal/reports/scheduler.go:233`  ·  **lens:** reports  ·  status: ✅ DONE (D-113, PR #99, prod v0.4.0-43-g7c206a9)  ·  **NOTE:** fixed by normalizing the seed to UTC INSIDE `nextCronTime` (`from = from.UTC()`) rather than the ledger's 3-call-site approach — DRY, covers all callers, testable via exported `NextCronTime`. Latent on this UTC prod; real for non-UTC self-hosted installs. Bundled with [4].
...
- **loc:** `server/internal/cluster/discovery.go:145`  ·  **lens:** cluster  ·  status: ✅ DONE (D-119, PR #111, prod v0.4.0-55-ge13eb1f)  ·  **NOTE:** added a dedup guard at the top of the poll loop — if the resolved key is already in `seen`, log + `continue` (the `seen` map already backed the stale-check; it now serves both roles). Two colliding DTOs → one `node_stats` emit + `NodeCount==1`; distinct nodes still emit each (positive control). Mutation-proven (drop the guard's `continue` → dedup test reddens `got 2 want 1`). Self-review (mechanical, LOW).
