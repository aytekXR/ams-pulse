...
## ▶ START HERE (next session = SESSION-103 clean start — the operator DELEGATED the go-to-market decisions; they are MADE (D-169). Two autonomous first-draft tasks remain: the Playwright demo rough-cut and the Ankush reply DRAFT. Everything else is operator-outbound.)

> **✅ Where we are (2026-07-25):** v0.4.1 is released, GHCR is public, and the anonymous clean-room evaluator install was verified end-to-end (D-168). The operator then delegated all remaining go-to-market decisions ("decide for me… good pricing… almost free first year… decide the rest"). **D-169 records them and the customer-facing docs are updated:**
> - **Pricing:** Free $0 / Pro $99 / Business $299 / Enterprise from $799 per month (annual = 2 months free); node ladder Free 1 / Pro 10 / Business 50 / Enterprise ∞.
> - **Launch campaign "Founding Operators":** near-free year 1 — Pro **$9/mo** & Business **$29/mo** for the first 12 months (first 6 months / 100 activations), Enterprise 90-day pilot + 25% off, Free stays free; auto-revert at renewal + 10% loyalty discount.
> - **Trial** 14-day Pro (graceful revert to Free); **support** community/email `support@beyondkaira.com` 2-day / priority 1-day / Enterprise 4-business-hour + named contact; **capacity** provisional (honest, benchmark pending the load lane); **demo** = loop rough-cut; **D-081** cleared.
>
> **SESSION-103 GOAL (autonomous, do these):**
> 1. **Demo rough-cut** — attempt a Playwright video/screenshot walkthrough of the six key screens (see `docs/marketplace/demo-video-script.md` + `screenshot-list.md`) against the live app, on a remapped port / isolated context, **never touching prod**. Land it as a first-draft asset for the operator to re-record with voiceover. If Playwright video capture is not feasible in this env, produce a scripted screenshot sequence instead and say so.
> 2. **Draft the Ankush reply** — a ready-to-send email/message per `docs/marketplace/developer-meeting-brief.md` (docs ready, request the developer meeting). DRAFT only — the operator sends it (their identity). Save it under `docs/marketplace/`.
> 3. Then the autonomous backlog is empty again — low-frequency gate, **read prod health every cycle** (ClickHouse `server_events` count + `/healthz` collector component; that read caught the last two live regressions).
>
> **Operator-outbound remainder (loop cannot do — `operator-expected.md` ★S103):** submit the listing, set up billing, provision the support mailbox, run the load lane on a PAYG AMS, record the demo final, send the Ankush reply, optional prod roll to 0.4.1, rotate secrets.
>
> **Guardrails:** every D-169 price is the operator's to override — do not treat them as immutable. NO code changed and NO prod roll in D-169 (prod stays v0.4.0-139, collector `ok`). Do NOT perform any outbound/marketplace action or send anything on the operator's behalf. `git status` + commit-by-path (a rogue restore hit an earlier session).

**Session 2026-07-24 (S102) result: D-167 — shipped the autonomous half of ROADMAP §2.45 (PR #206).** With v0.4.1 released and the listing path operator-gated, the highest-value autonomous item was §2.45's "cheapest first step": `/metrics` now exposes `pulse_collector_last_success_timestamp` and `pulse_collector_up` so a Prometheus user can alert on Pulse's *own* collector going blind — the notification gap the D-164 outage exposed (Pulse's internal alert engine can't, because it evaluates metrics derived from the collector and falls silent when the collector does). Docs `docs/guides/prometheus.md`; 4 tests; NO prod roll. The *built-in* Pulse-native self-alert rule stays OPEN in §2.45 (decision-gated — needs a maintenance-window/tier/channel ruling from the operator). Record: `decisions.md` D-167, `sessions/SESSION-102.md`. **After D-167 the autonomous engineering backlog is empty** — every remaining roadmap item is operator-decision-gated (§2.45 built-in rule, §2.44 `[FO-1]`, Dependabot), tooling-blocked (§2.12 Android SDK needs a JVM+Gradle toolchain, ABSENT on this host), or needs the operator's real-AMS env (§2.47). So the next session either (a) runs the clean-room verify once the operator flips GHCR, (b) executes a specific operator decision, or (c) low-frequency-gates while reading prod health.

**Session 2026-07-24 (S101) result: D-166 — acted on the independent pre-listing review, proved D-164 (and found the D-164 fix itself was broken), and CUT v0.4.1.** All the review's code blockers/fast-follows are fixed, merged (**PR #204 → `main` `e45ea8b`**), and released: **tag `v0.4.1` pushed → `release.yml` built + pushed `ghcr.io/aytekxr/ams-pulse:0.4.1` (multi-arch, cosign-signed)**. NO prod roll (prod stays `v0.4.0-139-gf9e9c69`, tier enterprise, collecting). Records: `decisions.md` D-166 (+ addendum), `sessions/SESSION-101.md`.

**★ The headline.** SESSION-101 was the D-164 PROVE-IT. The `/healthz` staleness signal proved sound (degrades at the 30 s floor with cause named, HTTP 200, recovers without restart — verified live in an isolated stack, prod untouched). But **`deployment.sh` step 6 was broken**: it grepped `"status":"ok"` against the whole `/healthz` body, which the clickhouse/meta_store components satisfy even while the collector is `degraded` — so a deploy against a dead AMS *passed the gate*, the exact failure S100 wrote it to catch. Fixed (assert the collector component specifically, fail-closed), re-proven live. Second catch: CI's own `e2e` gate flagged that the new alert-rule validator over-rejected `window_s:0` + omitted severity (both legitimate); corrected to reject only the review's actual hostile forms (negative window, non-empty unknown severity).

**What is DONE and RELEASED (do not re-do):** all of D-166 — blocker 2 (Business MaxNodes 5→50, monotonic ladder + test), blocker 3 (official pubkey embedded default on every compose path), **blocker 1 (v0.4.1 cut, tagged, GHCR image built)**, fast-follows 4-7 (source-test + channel SSRF, alert-rule validation, nil-scope token→`read`, alerting docs + `PULSE_BASE_URL`), lower + D-165 leftovers (main-port beacon validation, `/metrics`+SHA-256 warnings, **backup race §2.46**, SDK unload-beacon auth, onboarding-dup, Helm tag, MIGRATION.md scrub). Gates all green; CI + release both succeeded on `e45ea8b`.

**THE ONE REMAINING BLOCKER — operator-only, then autonomous:**
1. **Operator flips the GHCR package `ams-pulse` to Public.** No API for it (`PATCH /user/packages/...` → 404). The image `0.4.1` now EXISTS (so the flip is unblocked — the "wait for the image" precondition is met). See `operator-expected.md` ★S101 §1. Do NOT attempt it via CLI.
2. **The moment it is public, run the clean-room verify (autonomous, blocker 1's last mile):** in an isolated compose project, anonymous `docker pull ghcr.io/aytekxr/ams-pulse:0.4.1` (no auth) → `deploy/quickstart/install.sh` (or the quickstart compose) → assert `/healthz` collector `ok` → a live dashboard. Confirms an outside evaluator's exact path works end to end. Record it as the D-166 close-out.
3. **Until then**, back to the low-frequency gate. **Keep reading prod health every gate** (a ClickHouse `server_events` count + the `collector` component of `/healthz`) — that read, not the checklist, caught the last two live regressions. Prod is NOT on v0.4.1; it stays v0.4.0-139 unless/until the operator authorises a stamped `deployment.sh` roll (D-166 is not live in prod, only released).

**⚠ Concurrency (S100 §4) is RESOLVED** — the "second writer" was the D-165 review session, now done (PR #202 merged). Single-writer tree again. A **rogue `git restore` reverted mid-wave edits** this session (recovered); if edits vanish under you, `git status` + re-read before redoing, commit by explicit path.

**Operator queue (operator-expected.md ★S101):** ★ flip GHCR public (image now exists — unblocked). Carried unchanged: the permanent 2026-07-23 data hole, the 18-PR Dependabot ruling, final pricing sign-off (inversion fixed in code; numbers still PROPOSED), the 6-step submission sequence, rotate chat-exposed/VPS-group-readable secrets.

---

## (superseded — ACTED ON by D-166/S101 above; kept as the review's full itemisation for the submission gate) ▶ was: START HERE — ACT ON THE INDEPENDENT PRE-LISTING REVIEW · REVIEW-EXT-2026-07-24; verdict: DO NOT LIST TODAY

**2026-07-24 — an independent third-party pre-listing review ran the full marketplace-evaluator gauntlet** (outsider install from public docs only; release-integrity tag-vs-`main` audit; adversarial code review of auth / license / SSRF / ingest / alert-evaluator / SQL; a 20-claim docs-vs-code sweep; fresh-clone server+web+SDK suites; hostile alert-rule / SSRF / beacon payloads fired at an **isolated throwaway instance** on loopback ports 18190-91, unique project, torn down `-v` — prod never touched). **No ship, no prod roll, read-only.** It was run bias-controlled (formed findings BEFORE reading `docs/assessment/**` or D-164/165), and it independently re-derived most of what the vendor's own **D-165 release-readiness review already holds privately**, plus items D-165 missed and one D-165 claim it **contradicts with evidence**. Fold these into the submission gate — do NOT re-run the review as an arc.

**VERDICT: not listable today.** The installable/pinned artifact ≠ what the repo describes, and it is unreachable. Blockers 1-3 gate the listing; 4-7 are fast-follow. Most overlap the D-165 private open-list.

**BLOCKERS (before any listing):**
1. **Release integrity (Critical).** `v0.4.0` tag = `167f48d` = **D-089/S27**; HEAD is **141 commits** ahead; prod runs an *unreleased* source build `v0.4.0-139-gf9e9c69`. The released image PREDATES the whole hardening arc — verified-absent by ancestry: probe SSRF guard (D-130), cross-tenant leak fix (D-137), channel hardening (D-125), evaluator correctness (D-129), WS-auth (D-140). AND the GHCR package is still private: anonymous `docker pull …:0.4.0` → `unauthorized`, token 401 / manifest 403 → the quickstart is 100%-broken for evaluators. → **Cut v0.4.1 from HEAD, flip GHCR public, re-verify an anonymous clean-room install to a live dashboard.** (D-165 undersold this as "131 commits stale, misses D-157–160" — it misses the entire D-090→D-165 security arc.)
2. **Pricing inversion (High).** `license.go:124` Pro `MaxNodes:10` vs `license.go:135` Business `MaxNodes:5`; `CheckNodeLimit` (`license.go:312`) fails closed; the public table `licensing-public.md:90-91` ships it ($299 Business < $99 Pro on node count). Prices are all "(PROPOSED)". Fix the ladder or the pricing before listing — a marketplace listing cannot ship a higher tier with fewer nodes.
3. **Paid licenses silently downgraded on the README compose path (High).** `deploy/.env.example:136` ships `PULSE_LICENSE_PUBKEY` COMMENTED; `real-ams.yml:76` defaults it empty; `license.go:192-193` then falls back to the *dev* pubkey (`3dab4e90…`) → real vendor-signed keys fail verify → silent Free tier, no error. Quickstart bakes the right key; the two documented paths disagree. Ship the pubkey in `.env.example` / default it in `real-ams.yml`.

**FAST-FOLLOW (code):**
4. **SSRF: source-test path unguarded (High).** `server.go:1986` — comment says "private/loopback intentionally allowed", installs no `ssrfguard.DialControl`; `POST /admin/sources/{id}/test` dials link-local (169.254.169.254) — blind SSRF (timing/error oracle). Also alert channels `channels/webhook.go:44`, `channels.go:257`. (D-165 knew about the *channels*, NOT the source-test path.)
5. **Alert-rule input never validated (Medium).** `alertRuleFromAPI` `server.go:2456` — live-confirmed **HTTP 201** for unknown metric, `operator:"banana"`, `window_s:-3600`, `severity:"apocalypse"`, `threshold:1e308`; the evaluator then routes unknowns to `default:continue` → the rule is stored, evaluated every tick, and **never fires, no log**. A monitoring product must reject these. Also: wildcard `node_down` permanently inert `wave2.go:163-169` (residual of "FIXED" BUG-011 — "alert if ANY node down" never fires), and alerts lock open on a ClickHouse/reader error `wave2.go:102-108` / `wave3.go:233-238`.
6. **nil-scope API token = full admin (Medium).** `canWrite` `server.go:737-739` returns true for empty scopes; `handleCreateToken` `server.go:2098-2107` leaves scopes nil when the field is omitted → `POST /admin/tokens {"kind":"api","name":"x"}` mints a silent admin token that can delete users / mint tokens.
7. **Docs D-165 claims fixed are still broken (Medium — CONTRADICTS D-165).** CHANGELOG [Unreleased]/D-165 says the alerting sweep "fixed every confirmed drift", but `alerting.md:79` still describes `ingest_bitrate_floor` as `S_bitrate<0.5` (code compares raw kbps) and `alerting.md:101-103` / `:127-129` / `:227` still ship the inverted `stream_offline threshold:0` examples (fire while the stream is HEALTHY). Only the summary table (`:480`) was corrected. Overall docs drift = **4/20**; also `PULSE_BASE_URL` (admin-guide.md:72) is wired to nothing (`serve.go:433` hardcodes `http://<listen>`) → broken alert deep-links.

**LOWER:** main-port `/ingest/beacon` skips `validateBeaconBatch` `server.go:2300-2401` (Pro+ / token-gated); token hashing → plain SHA-256 when `PULSE_SECRET_KEY` unset `meta.go:635` (no warning); `/metrics` open when `PULSE_METRICS_TOKEN` unset `server.go:1044-1055`; `viewer_drop_pct` compares an absolute count not a % (default fires at 0 viewers); public-repo infra leak — `deploy/MIGRATION.md` (TRACKED: VPS IP + SSH user), nginx vhosts, `docker-compose.override.yml:42-48` (documents admin API on :80 bypassing ufw). **Operational (NOT public-repo):** gitignored `deploy/.env` + `oguz-testing.md` carry the plaintext prod AMS password / `PULSE_SECRET_KEY` / license key group-readable on the VPS — rotate + remove; an evaluator cloning the public repo never receives them.

**CLEARED (adversarially checked — do NOT chase):** SQL is parameterized end-to-end (`group_by` is stored but only compared in-memory; POSTed `") UNION SELECT 1--"` is inert) — no injection; `ssrfguard` itself has no bypass (blocks link-local / IPv4-mapped / NAT64 / 0.0.0.0, dial-time so rebind- and redirect-safe); license expiry + forged/malformed keys fail CLOSED; beacon ingest requires a kind-checked token on BOTH ports and Free is 403 (live-verified). **Test hygiene is genuinely clean:** gofmt/vet clean, Go 26/26 pkgs, web `npm ci` no-override + 680/680, SDK 65/65 + 3.52 KB — every advertised gate passes, no flake, no override flags.

**KNEW vs NEW vs CONTRADICT** (for the submission gate, don't re-derive):
- **D-165 already holds privately:** release staleness, channel SSRF, alert-rule validation gap, `viewer_drop_pct`, main-port beacon validation, public-repo scrub, sendBeacon token-drop on unload, Helm tag `0.1.0`, onboarding-dup. (Credit — they also self-found the D-164 outage + the CH-backup race.)
- **NEW here (not in any vendor material):** GHCR-unpullable-today framing (blocker 1), the source-*test* SSRF `server.go:1986` (item 4), nil-scope=admin `server.go:737` (item 6), pubkey-empty→silent-Free on the README path (blocker 3), wildcard `node_down` residual + stuck-firing-on-outage (item 5), SHA-256 token fallback.
- **CONTRADICTS D-165 with evidence:** the "docs drift fully fixed" claim (item 7) and the "misses D-157–160" characterization (blocker 1).

**⚠ STILL CARRIED — D-164 PROVE-IT (was the prior START HERE, now folded here).** SESSION-101's task is unchanged and still owed: first confirm prod is still collecting, then — in an **isolated** throwaway project, never prod — prove the new `/healthz` collector-staleness signal actually fires against an unreachable AMS and that `deployment.sh` step 6 actually fails the deploy (D-164 changed a live health path + a deploy script; neither has been exercised against the failure it was written for). The concurrency rule still holds: **another session may be editing this repo — `git status` + re-read before every edit; never `git add -A`.** Operator queue (operator-expected.md ★S100) unchanged: the permanent 7h46m data hole (2026-07-23), a decision on the 18-PR Dependabot queue, and the 6-step submission sequence.

---

## (superseded — see the INDEPENDENT PRE-LISTING REVIEW · REVIEW-EXT-2026-07-24 above; its PROVE-IT task is carried into that block) ▶ was: START HERE (next session — execute `sessions/SESSION-100.md` §SESSION-101: PROVE the D-164 fixes, then back to the wait; ⚠ A SECOND SESSION IS WRITING THIS REPO)
