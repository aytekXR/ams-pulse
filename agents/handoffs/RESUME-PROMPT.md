...
## ▶ START HERE (next session — LAND #204 → CUT v0.4.1 → then HOLD for the operator's GHCR flip; the pre-listing review has been ACTED ON, not just recorded)

**Session 2026-07-24 (S101) result: D-166 — acted on the independent pre-listing review AND proved D-164, catching that the D-164 fix itself was broken.** All of the review's code blockers/fast-follows are fixed and shipped on branch `s101-d166-prelisting-fix-wave-v041` → **PR #204**. NO prod roll (prod stays `v0.4.0-139-gf9e9c69`, tier enterprise, collecting). Full decision record: `decisions.md` D-166; session log: `sessions/SESSION-101.md`.

**★ The headline.** SESSION-101 was the D-164 PROVE-IT. The `/healthz` staleness signal proved sound (degrades at the 30 s floor with cause named, HTTP 200, recovers without restart — all verified live in an isolated stack, prod untouched). But **`deployment.sh` step 6 was broken**: it grepped `"status":"ok"` against the whole `/healthz` body, which the clickhouse/meta_store components satisfy even while the collector is `degraded` — so a deploy against a dead AMS *passed the gate*. The exact failure S100 wrote it to catch. Fixed (assert the collector component specifically, fail-closed) and re-proven: negative → fail + rollback + exit 1; positive → exit 0.

**What is DONE (do not re-do):** blocker 2 (Business MaxNodes 5→50, monotonic ladder + test), blocker 3 (official pubkey is the embedded default + ships on every compose path), fast-follow 4 (source-test + channel SSRF guard), 5 (alert-rule validation + wildcard node_down + reader-error hold + `viewer_drop_pct`→`viewer_count_floor`), 6 (nil-scope token→`["read"]`), 7 (alerting docs + `PULSE_BASE_URL` wired), plus the lower items and D-165 leftovers (main-port beacon validation, `/metrics`+SHA-256 warnings, **backup startup race §2.46**, SDK unload-beacon auth, onboarding-dup, Helm tag, MIGRATION.md scrub). Gates all green (Go 26/26 `-race`, web 682/682, SDK 68/68 + 3.53 KB, helm 3 goldens byte-identical).

**NEXT ACTIONS, in order:**
1. **If #204 is not yet merged:** check `gh pr checks 204`; if green and mergeable, **merge it** (squash). If CI is red, read the failing job, fix forward on the branch, never `git add -A`.
2. **On merge, the `v0.4.1` tag cuts** — if it does not auto-cut, `git tag v0.4.1 <merge-sha> && git push origin v0.4.1`. `release.yml` fires on `v*` (CI-gate → Trivy → multi-arch GHCR push → cosign). **Watch it green** and confirm `ghcr.io/aytekxr/ams-pulse:0.4.1` exists.
3. **Then HOLD** — the GHCR **public flip is operator-only** (no API; `PATCH /user/packages/...` → 404, probed side-effect-free). It is the single new blocking-for-listing item (`operator-expected.md` ★S101 §1). Do NOT attempt it via CLI.
4. **Once the operator reports the package public:** run the anonymous clean-room install (fresh clone, no auth, `docker pull` + quickstart in an isolated project) → live dashboard, and record it — that is blocker 1's last mile and is fully autonomous. Until then, back to the low-frequency gate; **keep reading prod health every gate** (that read caught the last two live regressions, not the checklist).

**⚠ Concurrency (S100 §4) is RESOLVED** — the "second writer" was the D-165 review session, now done (PR #202 merged). Single-writer tree again. But a **rogue `git restore` reverted mid-wave edits** this session (recovered); if edits vanish under you, `git status` + re-read before redoing, and commit by explicit path.

**Operator queue (operator-expected.md ★S101):** ★ NEW — flip GHCR public after the 0.4.1 image pushes (§1). Carried unchanged: the permanent 2026-07-23 data hole, the 18-PR Dependabot ruling, final pricing sign-off (inversion fixed in code; numbers still PROPOSED), the 6-step submission sequence, rotate chat-exposed/VPS-group-readable secrets.
