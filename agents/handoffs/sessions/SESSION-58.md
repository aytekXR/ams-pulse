...
## ⚡ STANDING DIRECTIVE (operator, 2026-07-12) — carry into SESSION-59
...
## ★ Context — the MEDIUM/LOW batch (4 remain; all 6 HIGH + [7],[9],[10],[13],[15],[16] shipped)

Shipped: [6] D-110, [1]+[2] D-111, [3] D-112, [4]+[15] D-113, [5] D-114, [7] D-115, [9] D-116, [10] D-117,
[13] D-118, [16] D-119. **4 remain** — the ranked list with per-finding mechanism + fix + test seam lives in
**`RESUME-PROMPT.md` ▶ START HERE** and `agents/handoffs/S48-AUDIT-FINDINGS.md`. Suggested next: **[14]** beacon 413
heuristic (clean; `collector/beacon/beacon.go:352` — use `errors.As(err, &http.MaxBytesError)` instead of the
`len(body) >= maxBodyBytes-1` byte-count heuristic; add `errors` import; also remove the now-unreachable post-read
size check). **[11]** needs a SQL-text/real-CH test seam (fake conn is vacuous). **[12]** needs a migration (FIVE
...
- `git log --oneline origin/main -4` — S57 (D-119, PR #111) + its docs PR should be on `origin/main`.
- Prod should print **`v0.4.0-55-ge13eb1f`** (rollback image tag `pre-d119` → `v0.4.0-53-g500aabb`). `/healthz`
...
2. Mutation-prove every guard; positive control so the harness can't be vacuous. For DB-text bugs ([11]) a
   ...
   (RED test) over build-breaking ones (S55).
3. Independent review before merge; a compact 1–2 lens review OR careful self-review for a purely mechanical
   mutation-proven fix (S53/S54/S56/S57). A fix carrying a genuine SEMANTIC/product decision (S55's `mixed`) warrants
   the multi-lens adversarial workflow.
...
2. `decisions.md` **D-120** evidence — append EARLY.
3. RESUME-PROMPT ▶ START HERE → SESSION-59; ROADMAP-V2 §2.30 ledger; mark shipped findings ✅ in `S48-AUDIT-FINDINGS.md`.
4. REFRESH `docs/operator-expected.md`.
5. Write `sessions/SESSION-59.md`.
