...
## ⚡ STANDING DIRECTIVE (operator, 2026-07-12) — carry into SESSION-57
...
re-verify against the code; take the verified CORE, not the audit's literal suggested scope** (S49 [2] subtler than
its summary; S50/S51 narrowed; S55 [10] BROADENED — the audit's "any→byte-counter" would have over-claimed on a
mixed report). One scope per PR.

## ★ Context — the MEDIUM/LOW batch (6 remain; all 6 HIGH + [7],[9],[10],[15] shipped)

Shipped: [6] D-110, [1]+[2] D-111, [3] D-112, [4]+[15] D-113, [5] D-114, [7] D-115, [9] D-116, [10] D-117.
**6 remain** — the ranked list with per-finding mechanism + fix + test seam lives in **`RESUME-PROMPT.md` ▶ START
HERE** and `agents/handoffs/S48-AUDIT-FINDINGS.md`. Suggested next: **[13]** clickhouse per-item `PrepareBatch`
(clean; `mockConn`/`mockBatch` scaffolding already in `drain_test.go`), then **[16]** dup node_stats, **[14]**
beacon 413. **[11]** needs a SQL-text/real-CH test seam (fake conn is vacuous). **[12]** needs a migration (FIVE
places, next = 0005) and **[8]** needs product-viability verification (may be operator/contract-gated) — do last.

## ⛔ At open — verify, do not assume (D-095)

- `git log --oneline origin/main -4` — S55 (D-117, PR #107) + its docs PR should be on `origin/main`.
- Prod should print **`v0.4.0-51-ge5577f7`** (rollback image tag `pre-d117` → `v0.4.0-49-g6d60f53`). `/healthz`
...
- **★ RUN `gofmt -l .` BEFORE EVERY PUSH** — CI's `server` job has a gofmt gate the local `go build && go vet` misses; a formatting nit fails CI (~30 s) and costs a force-push round-trip (S54). Gate: `sh -c 'D=$(gofmt -l .); [ -z "$D" ] && go build ./... && go vet ./... && go test ./... || { echo DIRTY: $D; exit 1; }'`. (Memory: `ci-gofmt-gate`.)
- **Mutation-prove on a COPY** mounted read-only at `/repo`: `cp -a /repo/server /mut/server && cp -a /repo/contracts /mut/contracts`; mutate `/mut`; test there. **Target precisely**; a replacement ending in `{` breaks perl `{}` delimiters → use `#` (S46/S53/S54). Prefer mutations that keep compilation valid so the failure is a RED test, not a build error (S55). Never `git reset/checkout/stash/restore <path>` (D-096); `git restore --staged` is fine.
- **CodeQL is a required check.** **Contract change? `cd web && npm run gen:api`** — CI's `web` job has a **types-drift guard** (`git diff --exit-code src/lib/api/schema.d.ts`); regenerate with `npm ci --legacy-peer-deps` (lockfile-pinned `openapi-typescript`, node 22) so your `schema.d.ts` matches CI byte-for-byte (S55). **New migration? FIVE places** (0004 → next 0005; **[12] needs this**). **Playwright** in `mcr.microsoft.com/playwright:v1.61.1-jammy` after `npm run build`.
- **`deploy/config/Caddyfile.prod` stays UNCOMMITTED** (clean status = FAILURE signal). **Never restart/fix AMS.**
- **Prod deploy LOCAL** — `deploy/runbooks/upgrade-rollback.md`: 5-overlay `DC_ARGS`; **rollback point is a DOCKER image tag** `docker tag pulse-prod-pulse:latest pulse-prod-pulse:pre-dNNN` (NOT a git tag) → backup (`exec -T backup … once`, rc 0) → STAMPED `build --build-arg VERSION=$(git describe --tags --always) COMMIT=… BUILD_DATE=…` (the build takes >2 min → run it backgrounded) → assert stamp ≠ dev → `up -d` (no `--build`) → smoke (healthz, version, signed webhook 200, limits 512M/0.5cpu, logs clean). Roll forward ONLY if server/web *source* changed. Admin token in gitignored `oguz-testing.md` (side-effect-free requests only).

## Binding lessons (carry into every wave)

1. Verify product-viability AND mechanism before building; take the verified CORE — which may be NARROWER (S50/S51)
   or BROADER (S55) than the audit's literal scope. Trace an existing test before trusting it (S49 [2]). Some
   findings ([8], [12]) need product/ops verification before coding.
2. Mutation-prove every guard; positive control so the harness can't be vacuous. For DB-text bugs ([11]) a
   ...
   (RED test) over ones that break the build (S55).
3. Independent review before merge; a compact 1–2 lens review OR careful self-review for a purely mechanical
   mutation-proven fix. A fix that carries a genuine SEMANTIC/product decision (e.g. S55's new `mixed` value)
   warrants the multi-lens adversarial workflow (S48/S50–S55).
4. Positive allowlists over blocklists (D-098). Respect documented contract/design even when an audit disagrees —
   and when introducing a new API-returned value, document it in the contract (S55: OpenAPI description + regen).
...
2. `decisions.md` **D-118** evidence — append EARLY.
3. RESUME-PROMPT ▶ START HERE → SESSION-57; ROADMAP-V2 §2.30 ledger; mark shipped findings ✅ in `S48-AUDIT-FINDINGS.md`.
4. REFRESH `docs/operator-expected.md`.
5. Write `sessions/SESSION-57.md`.
